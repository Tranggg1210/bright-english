...
import PreviewMultipleChoice from "@src/components/molecules/exercise/preview-multiple-choice";
import MultipleChoiceForm from "@src/components/molecules/exercise/multiple-choice-form";
...
function MultipleChoiceExercise() {
  ...
  const loaderExerciseById = async () => {
    try {
      const { data }: { data: { exercise: ExerciseType } } = await dispatch(
        getExerciseById({
  ...
  const handleSave = async () => {
    try {
      ...
      const sanitizedQuestions = questions.map(({ id, _id, ...rest }) => rest);
