...
import Volumn from "@public/images/volume.png";
import Image from "next/image";
import useAudio from "@src/hooks/useAudio";

function FlashcarDetail({ item }: { item: IVocabulary }) {
  const { isPlay, speak, stop } = useAudio();

  const renderNote = (status: boolean) => (
    <svg
  ...
  return (
    <div className="flashcard-detail">
      <div className="fd-header">
        <div className="fdh-left">
          {renderNote(item?.isLearn || false)}
          <span 
            style={{
              color: item?.isLearn ? "#ff8400" : "rgb(163 171 192)"
            }}
          >{item?.isLearn ? "Đã học" : "Chưa học"}</span>
        </div>
        <div className="fdh-right"
            style={{
                background: isPlay ? "#ff8400" : "#ffe5b4"
            }}
        ...
        >
          <Image src={Volumn} alt="icon" width={24} height={24} />
        </div>
      </div>
      <div className="fd-body">
        <h1 className="fdb-title">
          {item.word} - {item.translate}
        </h1>
        <div className="fdb-image">
          <Image src={item.image} alt="icon" width={350} height={350} />
        </div>
      </div>
    </div>
  );
